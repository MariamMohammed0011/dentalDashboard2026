...
export const useUpdateUserStatus = (queryKeysToInvalidate = []) => {
  ...
  const mutation = useMutation({
    mutationFn: async ({ id, status, type }) => {
      ...
      // 2. تحديد نوع الحساب (طبيب أو مخبر) لتحديد المسار الفرعي للطلب
      // نقبل القيمتين 'doctor' و 'dentist' ونوحدها لـ 'doctor' كما تتوقع الـ API
    ...
    onSuccess: (data, variables) => {
      // 3. تحديث الكاش وإعادة جلب البيانات للمفاتيح الممررة أو للمفاتيح الافتراضية
      if (queryKeysToInvalidate && queryKeysToInvalidate.length > 0) {
        queryKeysToInvalidate.forEach(key => {
          queryClient.invalidateQueries({ queryKey: [key] });
        });
      } else {
        // افتراضياً: نقوم بتحديث كاش كافة القوائم المعنية لضمان التزامن الكامل
      ...
      // 4. عرض رسالة نجاح مخصصة بناءً على الحالة الجديدة

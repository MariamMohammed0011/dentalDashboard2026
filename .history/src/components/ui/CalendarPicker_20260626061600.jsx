...
// Helper to format Date object for user display (Arabic format)
...
const CalendarPicker = ({ value, onChange, placeholder = 'اختر التاريخ', disabled = false, className = '' }) => {
  ...
  // Internal calendar view month and year
  const [viewMonth, setViewMonth] = useState(new Date().getMonth());
  const [viewYear, setViewYear] = useState(new Date().getFullYear());

  // Sync internal view with selected value when popover opens
  useEffect(() => {
    if (isOpen && value) {
  ...
  // Generate days in viewMonth
  const daysInMonth = new Date(viewYear, viewMonth + 1, 0).getDate();
  
  // Starting day of the week (0 = Sunday, 1 = Monday, ..., 6 = Saturday)
  const firstDayIndex = new Date(viewYear, viewMonth, 1).getDay();
  
  // We want grid to start on Saturday:
  // Saturday = index 0, Sunday = index 1, ..., Friday = index 6
  // Shift index: (firstDayIndex + 1) % 7
  ...
  // Generate Year dropdown options (e.g. current year - 2 to current year + 10)
  const currentYearNum = new Date().getFullYear();
  const yearOptions = Array.from({ length: 15 }, (_, i) => currentYearNum - 2 + i);

  // Parse current selected value to check active styling

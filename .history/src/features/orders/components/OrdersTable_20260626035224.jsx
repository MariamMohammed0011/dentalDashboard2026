...
const OrdersTable = ({ orders, isLoading }) => {
  ...
  // دالة مساعدة لتنظيف وتنسيق التاريخ للغة الإنجليزية بشكل آمن
  const formatEnglishDate = (dateString) => {
    if (!dateString) return '—';
    try {
      // تنظيف الأرقام الزائدة بعد النقطة لمنع خطأ Invalid Date
        ...
        <table className="w-full text-center border-collapse table-auto">
          ...
          <tbody>
            {orders?.map((order) => (
              ...
              >
                <td className="px-4 py-4 text-gray-600 text-sm border-l border-gray-200 last:border-l-0">{order.orderId}</td>
                <td className="px-4 py-4 text-gray-600 text-sm border-l border-gray-200 font-medium">{order.dentistName}</td>
                <td className="px-4 py-4 text-gray-600 text-sm border-l border-gray-200 font-medium">{order.labName}</td>
                ...
                {/* تعديل التاريخ هنا للكمبيوتر */}
                <td className="px-4 py-4 font-bold text-gray-400 text-sm border-l border-gray-200">
                  {formatEnglishDate(order.createdAt)}
                </td>
              ...
              <div className="flex justify-between items-center mb-4">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center text-blue-600">
                    <span className="text-[10px] font-black">#{order.orderId}</span>
                  </div>
                  {/* تعديل التاريخ هنا للجوال */}
                  <span className="text-[12px] text-gray-400 flex items-center gap-1" dir="ltr">
                    <Clock size={12} />
                    {formatEnglishDate(order.createdAt)}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[14px] font-bold text-gray-700">{order.dentistName}</span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-[11px] text-gray-400">حالة الطلب</span>
                    <span className="text-[13px] font-bold text-primary bg-primary/5 px-2 py-0.5 rounded-lg mt-1">{order.status}</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-50">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[12px] font-semibold text-gray-600 truncate">{order.labName}</span>
                    </div>
                  </div>
                </div>
              </div>

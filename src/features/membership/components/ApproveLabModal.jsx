...
import { X, DollarSign, Gift, Loader2, CheckCircle2 } from 'lucide-react';

export default function ApproveLabModal({ isOpen, labName, onClose, onConfirm }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'حدث خطأ أثناء الموافقة على المخبر');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return createPortal(
    <AnimatePresence>
      <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 overflow-hidden" dir="rtl">
        {/* Overlay */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <div className="relative p-6 sm:p-7 bg-gradient-to-br from-emerald-500 via-teal-600 to-emerald-700 text-white flex items-center justify-between overflow-hidden">
            <div className="z-10">
              <h2 className="text-xl sm:text-2xl font-black tracking-tight">الموافقة على المخبر</h2>
              {labName && (
                <p className="text-emerald-100 text-xs font-bold mt-1">{labName}</p>
              )}
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-5">
            {/* Info Notice */}
            <div className="p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200/60 dark:border-emerald-900/40 flex items-start gap-2.5">
  <Gift size={16} className="text-emerald-500 shrink-0 mt-0.5" />
  <p className="font-zain text-xs font-bold text-emerald-800 dark:text-emerald-300 leading-relaxed">
    عند الموافقة، سيتم تفعيل حساب المختبر فوراً مع منح فترة تجريبية مجانية لمدة شهر كامل.
    <br className="hidden sm:block" />
    {' '}يرجى تحديد قيمة الاشتراك الشهري أدناه، والتي ستُطبّق تلقائياً بعد انتهاء الفترة المجانية.
  </p>
</div>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[11px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-wider flex items-center gap-1.5 mr-1">
                ...
                سعر الاشتراك الشهري بعد الفترة المجانية <span className="text-rose-500">*</span>
              </label>

        ...
        {isLoading ? (
          ...
          <AnimatePresence mode="popLayout">
            {notifications.map((notif) => (
              ...
              >
                {/* ID */}
                <div className="w-[10%] font-mono text-xs text-gray-500 font-bold">
                  #{notif.id}
                </div>
                ...
                <div className="w-[15%] flex items-center gap-2">
                  ...
                  <span className={`text-[11px] font-black px-2.5 py-1 rounded-full border ${notifBadge[notif.type]?.color || 'bg-slate-50 text-slate-500'
                    }`}>
                ...
                {/* Text Content */}
                <div className="w-[47%] text-right font-medium text-text-main dark:text-gray-250 text-[13px] pr-2 pl-4 truncate">
              ...
              {notifications.map((notif) => (
                ...
                >
                  <div className="flex justify-between items-center pb-2 border-b border-slate-100 dark:border-slate-800/50">
                    ...
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-md border ${notifBadge[notif.type]?.color || 'bg-slate-50 text-slate-500'
                      }`}>
                      {notifBadge[notif.type]?.label || "تنبيه"}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-2.5 items-start">
                    ...
                    <div className="text-right flex flex-col gap-1 flex-grow">
                      <p className={`text-xs leading-relaxed ${!notif.read ? 'font-black text-gray-800 dark:text-white' : 'text-gray-500 dark:text-gray-400 font-medium'
                        }`}>
                        {notif.text}
                      </p>

            ...
            <div className="p-5 sm:p-7 flex flex-col gap-5 text-right overflow-y-auto flex-grow custom-scrollbar">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-3xl border border-gray-100">
                <div className="flex flex-col gap-1">
                  <span className="text-xs text-gray-400 font-bold">{t('ads.viewAdModal.adIdLabel')}</span>
                  <span className="text-sm font-extrabold text-gray-700">#{selectedAd.id}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-xs text-gray-400 font-bold">{t('ads.addAdForUserModal.audienceLabel')}</span>
                  <span className="text-sm font-extrabold text-[#367AFF]">
                    {selectedAd.type === 'labs'
                      ? t('ads.addAdForUserModal.audienceLabs')
                      : selectedAd.type === 'both'
                      ? t('ads.addAdForUserModal.audienceBoth')
                      : selectedAd.type === 'dentists'
                      ? t('ads.addAdForUserModal.audienceDentists')
                      : t('ads.notSpecified')}
                  </span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-xs text-gray-400 font-bold">{t('ads.viewAdModal.activationStatusLabel')}</span>
                  <span className="text-sm font-extrabold">
                    {selectedAd.status === 'active' ? (
                      <span className="text-blue-600">{t('ads.viewAdModal.activeNowStatus')}</span>
                    ) : (
                      <span className="text-rose-500">{t('ads.adsList.inactiveStatus')}</span>
                    )}
                  </span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-xs text-gray-400 font-bold">{t('ads.viewAdModal.reviewStatusLabel')}</span>
                  <div>
                    {selectedAd.approvalStatus === 'pending' && <span className="text-amber-500 font-bold text-sm">{t('common.pending')}</span>}
                    {selectedAd.approvalStatus === 'approved' && <span className="text-blue-600 font-bold text-sm">{t('ads.viewAdModal.approvedStatus')}</span>}
                    {selectedAd.approvalStatus === 'rejected' && <span className="text-rose-500 font-bold text-sm">{t('common.rejected')}</span>}
                  </div>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-xs text-gray-400 font-bold">{t('ads.viewAdModal.campaignPriceLabel')}</span>
                  <span className="text-sm font-extrabold text-gray-800">
                    {selectedAd.price ? t('ads.viewAdModal.campaignPriceValue', { price: Number(selectedAd.price).toLocaleString() }) : t('ads.notSpecified')}
                  </span>
                </div>
                {selectedAd.expiresAt && (
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-gray-400 font-bold">{t('ads.addAdForUserModal.expiresLabel')}</span>
                    <span className="text-sm font-extrabold text-gray-700">{selectedAd.expiresAt}</span>
                  </div>
                )}
                {selectedAd.raw?.createdAt && (
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-gray-400 font-bold">{t('ads.viewUserModal.createdAt')}</span>
              ...
              {selectedAd.images && selectedAd.images.length > 1 && (
                <div className="flex flex-col gap-2">
                  <span className="text-xs text-gray-400 font-bold">{t('ads.viewAdModal.imagesCountLabel', { count: selectedAd.images.length })}</span>
                  <div className="flex flex-wrap gap-2.5">
                    {selectedAd.images.map((img, idx) => (
                      ...
                      >
                        <img
                          src={img}
                          alt={t('ads.viewAdModal.imageAltPrefix', { index: idx + 1 })}
            ...
            <div className="flex gap-3 justify-end items-center p-4 sm:p-5 bg-gray-50/50 border-t border-gray-100 flex-shrink-0">
              ...
              {selectedAd.approvalStatus === 'pending' && !selectedAd.isAdminCreated && !selectedAd.hasPrice && (
                <>
                  ...
                  >
                    {t('ads.viewAdModal.rejectAdButton')}
                  </button>
                  <button
                    onClick={() => {
                  ...
                  >
                    {t('ads.viewAdModal.approveAdButton')}
                  </button>
                </>
              )}
            </div>

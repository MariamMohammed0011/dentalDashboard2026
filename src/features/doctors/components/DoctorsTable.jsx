...
import { MapPin, Phone, Mail, Calendar, Building2, Users, Loader2, ChevronDown, Check, X, CheckCircle2, AlertTriangle, Clock } from 'lucide-react';
        ...
        {selectedDocForStatus && (
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setSelectedDocForStatus(null)}
            ...
            >
              ...
              <div className="mx-6 mt-5 p-3.5 rounded-2xl bg-sky-50/40 dark:bg-slate-800/30 border border-sky-100/20 dark:border-slate-800/40 flex items-center gap-3">
                <div className="w-11 h-11 rounded-2xl overflow-hidden border border-primary/20 shrink-0 bg-sky-100 dark:bg-sky-950 flex items-center justify-center font-bold">
                  <img
                    src={`https://ui-avatars.com/api/?name=${encodeURIComponent(selectedDocForStatus.name || 'طبيب')}&background=e0f2fe&color=367AFF&bold=true&size=64`}
                    alt={selectedDocForStatus.name}
                    className="w-full h-full object-cover"
                  />
                </div>
                <div className="flex flex-col min-w-0">
                  <span className="font-extrabold text-gray-800 dark:text-gray-100 text-xs leading-tight">{selectedDocForStatus.name}</span>
                  <span className="text-[10px] text-gray-400 dark:text-slate-400 font-semibold mt-1 flex items-center gap-1">
                    ...
                    {selectedDocForStatus.clinicName || 'عيادة غير محددة'}
                  </span>
                </div>
              </div>
